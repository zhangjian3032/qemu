...
{
    /* TODO: introduce an address space */
...
static int aspeed_i2c_bus_recv(AspeedI2CBus *bus)
{
    int ret;
    ...
    if (bus->cmd & I2CD_RX_DMA_ENABLE) {
        while (bus->dma_len) {
            ...
            /* TODO: introduce an address space */
            cpu_physical_memory_write(bus->dma_addr, &ret, 1);
            bus->dma_addr++;
            bus->dma_len--;
        }
        ...
        ret = i2c_recv(bus->bus);
        if (ret < 0) {
            bus->buf = 0xff << I2CD_BYTE_BUF_RX_SHIFT;
            goto out;
        }
