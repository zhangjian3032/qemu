                                 ...
                                 unsigned size)
{
    ...
    if (addr + size > sizeof(s->regs)) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: Out of bounds write: %"HWADDR_PRIx" for %u\n",
                      __func__, addr, size);
        return;
    }
...
static void aspeed_apb2opb_realize(DeviceState *dev, Error **errp)
{
    ...
    qbus_create_inplace(&s->opb[0], sizeof(s->opb[0]), TYPE_OP_BUS,
                        DEVICE(s), NULL);
    qbus_create_inplace(&s->opb[1], sizeof(s->opb[1]), TYPE_OP_BUS,
                        DEVICE(s), NULL);
