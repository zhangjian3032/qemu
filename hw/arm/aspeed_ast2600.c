...
static void aspeed_soc_ast2600_realize(DeviceState *dev, Error **errp)
{
    ...
    sysbus_mmio_map(SYS_BUS_DEVICE(&s->adc), 0, sc->memmap[ASPEED_ADC]);
    ...
    if (serial_hd(1)) {
        ...
        serial_mm_init(get_system_memory(), sc->memmap[ASPEED_VUART], 2,
    ...
    if (serial_hd(2)) {
        ...
        serial_mm_init(get_system_memory(), sc->memmap[ASPEED_UART1], 2,
    ...
    sysbus_mmio_map(SYS_BUS_DEVICE(&s->pwm), 0, sc->memmap[ASPEED_PWM]);
